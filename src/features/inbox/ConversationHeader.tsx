  ...
  return (
    <div className="h-10 border-b flex items-center px-3 gap-2">
      ...
      <div className="min-w-0 flex-1">
        {contactDeal ? (
          <button
            ...
            className="text-sm font-medium truncate text-left hover:text-primary hover:underline underline-offset-2 transition-colors block max-w-full"
            title="Abrir Lead"
          >
            {c?.display_name ?? "Sem nome"}
          </button>
        ) : (
          <div className="text-sm font-medium truncate">{c?.display_name ?? "Sem nome"}</div>
        )}
        <div className="text-[10px] text-muted-foreground truncate leading-tight">
          {c?.phone_e164}
          {contactDeal && <span className="ml-1 text-primary/70">· Lead vinculado</span>}
        </div>
      </div>
      ...
      <div className="ml-auto flex items-center gap-0.5 shrink-0">
        ...
        <Button size="sm" className="h-7 text-xs gap-1 px-2" onClick={() => setOpenDeal(true)}>
          <Plus className="h-3 w-3" />
          {contactDeal ? "Lead" : "Novo"}
        </Button>
