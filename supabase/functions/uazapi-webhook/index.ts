  ...
  try {
    ...
    for (const m of msgs) {
      ...
      const { error: msgError } = await sb.from("messages").insert({
        ...
        type: inferType(m),
        payload: { body: extractText(m), external_id: externalId, from_phone: fromMe ? normalizePhone(myNumber) : phone, to_phone: fromMe ? phone : normalizePhone(myNumber), is_group: isGroup, group_jid: isGroup ? String(remote) : null, participant: isGroup ? participant : null, participant_phone: isGroup ? normalizePhone(participant) : null },
